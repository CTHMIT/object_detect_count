...
class C2f(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, n: int = 1) -> None:
        ...
        self.cv2 = Conv(hidden_channels * (1 + n), out_channels, kernel_size=1, stride=1, padding=0)
        self.m = nn.ModuleList(Bottleneck(hidden_channels, hidden_channels) for _ in range(n))
...
class Detect(nn.Module):
    def __init__(self, nc: int = 80, anchors: Tuple[int, ...] = (), ch: Tuple[int, ...] = ()) -> None:
